...
$(document).ready(function ()
{
    var form = $("#Registro");
    bindSelectEstado();
    $('#contenedorRegistro').addClass('animated fadeInDownBig');
    $("#contenedorRegistro").steps({
    //form.children("div").steps({
        ...
        onStepChanging: function (event, currentIndex, newIndex)
        {
            ...
            if ($("#cboxPersonaMoral").is(":checked") && currentIndex === 1 && $("#selectEstado option:selected").val()  != "default" &&  $("#selectMunicipio option:selected").val() != "default" &&  $("#selectLocalidad option:selected").val() != "default" && $("#txtNombreComprador").val() != "" && $("#txtRFComprador").val() != ""
            ...
            else if ( $("#selectEstado option:selected").val()  != "default" &&  $("#selectMunicipio option:selected").val() != "default" &&  $("#selectLocalidad option:selected").val() != "default" && $("#cboxPersonaFisica").is(":checked") && currentIndex === 1 && $("#txtNombreComprador").val() != "" && $("#txtRFComprador").val() != "" && $("#txtDireccionComprador").val() != "" && $("#txtColoniaComprador").val() != "" && $("#txtCPComprador").val() != "" &&
             $("#txtEmailComprador").val() != " " && $("#txtTelComprador").val() != "")
            {
                return true;
            }

            if ($("#selectEstado option:selected").val() === "default")
            {

                $("#selectEstado ").addClass('has-error')
            }
            if ($("#selectMunicipio option:selected").val() === "default")
            {

                $("#selectMunicipio").addClass('has-error')
            }
            if ($("#selectLocalidad option:selected").val() === "default")
            {
        ...
        onStepChanged: function (event, currentIndex, priorIndex) {
            ...
            if (currentIndex === 1 && $("#cboxPersonaMoral").is(":checked")) {
                ...
                $("#panelMasUsuarios").css('visibility', 'show');
            }
...
function configUpload()
{
    ...
    else
    {
        ...
        contacto.Apellidos = $("#txtApellidosContacto1").val();
        contacto.Email = $("#txtEmailContacto1").val();
        contacto.Telefono = $("#txtTelContacto1").val();
        contactos.push(contacto);
    }
...
function enviarDatosC()
{
    ...
    comprador.Email = $("#txtEmailComprador1").val();
    comprador.Telefono = $("#txtTelComprador1").val();
...
function mostrarForm2() {
    if ($("#cboxPersonaMoral").is(":checked")) {
        $("#panelMasUsuarios").css('display', 'block');
        ...
        $("#contenedor-btnContactos").empty().append('<button type="button" id="btnNuevoContacto" class="btn btn-success" data-toggle="modal" data-target="#myModalContacto" >Nuevo</button>')
...
$("#btnNuevoC").click(function ()
{
    ...
    if (tableContacto.rows().count() == 0 && $("#txtNombreContacto").text.length > 0 && $("#txtApellidosContacto").text.length > 0 && $("#txtEmailContacto").text.length > 0 && $("#txtTelContacto").text.length > 0)
    {
        ...
        var txtApellidosContacto = $("#txtApellidosContacto1").val();
        var txtEmailContacto = $("#txtEmailContacto1").val();
        var txtTelContacto = $("#txtTelContacto1").val();
...
function validarCheck()
{
    if ($("#cboxPersonaFisica").is(":checked"))
    {
        ...
        $("#legend-empresa").text('');
        $("#content-empresa").empty();
        $("#panelMasUsuarios").css('display', 'none');
    }
...
//        var expresionrRFCC = '^([A-ZÑ\x26]{3,4}([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])([A-Z]|[0-9]){2}([A]|[0-9]){1})?$';
...
function validarNombreContacto1() {
    ...
    if (fNombreComprador.length > 0) {
        ...
        $("#content-NombreContacto").removeClass("form-group has-success");
        $("#content-NombreContacto").addClass("form-group has-error");
        $("#msjValidacionContacto1").text("El campo no puede estar vacío");
    }
}
...
function validarApellidosContacto1() {
    ...
    if (fNombreComprador.length > 0) {
        ...
        $("#content-ApellidosContacto1").removeClass("form-group has-success");
        $("#content-ApellidosContacto1").addClass("form-group has-error");
        $("#msjValidacionApContacto1").text("El campo no puede estar vacío");
    }
}
...
function validarTelContactoN() {
    ...
    if (fTelComprador.length > 0) {
        ...
        if (!expresionrTelC.test(fTelComprador)) {
            ...
            $("#msjValidacionTelContactoN").text("El formato de teléfono es incorrecto");
            $("#txtTelContactoN").val(" ");
